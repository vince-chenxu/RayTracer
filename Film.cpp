...
void Film::commit(Sample& sample, Color color)
{
    ...
    //color.print();
    c.rgbRed = min((color.b * 255.0f), 255.0f);
    c.rgbGreen = min((color.g * 255.0f), 255.0f);
    c.rgbBlue = min((color.r * 255.0f), 255.0f);
    FreeImage_SetPixelColor(img, (unsigned int)sample.x, height - (unsigned int)sample.y - 1, &c);
}
